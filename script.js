        ...
        if (id && !id.startsWith('offline_')) {
          ...
          if (table === 'inventory' || table === 'suppliers' || table === 'purchase_orders') {
            ...
            if (index !== -1) {
              ...
              utils.saveToLocalStorage(`${table}_${data.section || section}`, dataArray);
            }
          }
        }
...
const cartManager = {
  ...
  completeCheckout: () => {
    ...
    dataManager.saveDataToSupabase('sales', saleRecord).then(() => {
      ...
      // Save updated stats
      const salesDataForDB = {
        id: section,
        totalSales: dataStores.salesData[section].totalSales,
        totalTransactions: dataStores.salesData[section].totalTransactions,
        topItem: dataStores.salesData[section].topItem,
        dailyTransactions: dataStores.salesData[section].dailyTransactions,
        profit: dataStores.salesData[section].profit
      };
      dataManager.saveDataToSupabase('sales_data', salesDataForDB, section);
...
const purchaseOrderManager = {
  ...
  addNewPurchaseOrder: () => {
    ...
    // Store productId locally for later use (when receiving order)
    newPurchaseOrder.productId = productId;
